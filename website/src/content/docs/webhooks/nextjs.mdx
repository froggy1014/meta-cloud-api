...
Both App Router and Pages Router adapters use singleton caching per `phoneNumberId`. This is especially important in Next.js where API routes can be re-evaluated during HMR in development — without caching, handlers would be registered multiple times.
...
Use `destroy()` to clear the cache and remove all handlers (useful in tests):

```typescript
const whatsapp = webhookHandler({ /* ... */ });
whatsapp.processor.onText(async (client, msg) => { /* ... */ });
