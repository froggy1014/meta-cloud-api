...
interface WebhookProcessor {
    ...
    // Webhook field handlers — Account & Business
    ...
    // Webhook field handlers — Phone Numbers
    ...
    // Webhook field handlers — Templates
    ...
    // Webhook field handlers — Calls
    onCalls(handler: CallsHandler): void;

    // Webhook field handlers — Groups
    ...
    // Webhook field handlers — Messaging
    ...
    // Handler removal
    offMessage(type: MessageTypesEnum): void;
    offStatus(): void;
...
All webhook handlers receive a third `context` parameter with the original HTTP request details. This is useful for signature verification, request forwarding, or logging.

```typescript
import type { WebhookHandlerContext } from 'meta-cloud-api';

processor.onText(async (whatsapp, processed, context: WebhookHandlerContext) => {
    // Access original request details
    console.log('Method:', context.method);
    console.log('URL:', context.url);
    console.log('Signature:', context.headers.get('x-hub-signature-256'));

    // The raw body is available for signature verification
    console.log('Raw body:', context.rawBody);

    await whatsapp.messages.text({
        to: processed.message.from,
        body: 'Hello!',
    });
});
```

The `WebhookHandlerContext` type:

```typescript
type WebhookHandlerContext = {
    headers: Headers;   // Original request headers
    rawBody: string;    // Original request body string
...
Use `onRaw` to receive the full, unprocessed webhook payload. Optionally filter by specific webhook fields:

```typescript
// Receive all raw webhook payloads
processor.onRaw(async (whatsapp, payload, context) => {
    console.log('Raw payload:', JSON.stringify(payload));
    // Forward to analytics, logging, etc.
});

// Or filter to specific webhook fields only
processor.onRaw(async (whatsapp, payload, context) => {
    console.log('Messages payload:', JSON.stringify(payload));
}, ['messages', 'account_update']);
```

### Handler Removal

Remove individual handlers or clear all handlers at once:
...
// Remove pre/post process handlers
...
This is useful for dynamic handler management, testing, or reconfiguring handlers at runtime.
    ...
    for (const call of value.calls) {
        switch (call.event) {
            case 'connect':
                ...
                break;
            case 'call_status':
                console.log(`Call ${call.id}: ${call.call_status}`);
                break;
            case 'terminate':
...
processor.onGroupLifecycleUpdate(async (whatsapp, { value }) => {
    for (const group of value.groups) {
        if (group.type === 'group_create') {
            console.log(`Group created: ${group.group_id} - ${group.subject}`);
        } else if (group.type === 'group_delete') {
...
// Participant changes
processor.onGroupParticipantsUpdate(async (whatsapp, { value }) => {
    for (const group of value.groups) {
        if (group.type === 'group_participants_add') {
            console.log(`Added: ${group.added_participants?.map(p => p.wa_id)}`);
        }
    }
});

// Group suspension
processor.onGroupStatusUpdate(async (whatsapp, { value }) => {
    for (const group of value.groups) {
        if (group.type === 'group_suspend') {
            console.warn(`Group ${group.group_id} suspended!`);
        }
    }
});
```

### Messaging Handover Handlers

```typescript
// Thread control passed to another app
processor.onMessagingHandovers(async (whatsapp, { value }) => {
    console.log('Handover from:', value.sender.phone_number);
    if (value.control_passed) {
        console.log('Metadata:', value.control_passed.metadata);
    }
});

// Standby mode (not the current thread owner)
processor.onStandby(async (whatsapp, { value }) => {
    console.log('Standby notification received');
});

// User preference changes (e.g., marketing opt-out)
processor.onUserPreferences(async (whatsapp, { value }) => {
    for (const pref of value.user_preferences) {
        if (pref.category === 'marketing_messages' && pref.value === 'stop') {
            console.log(`User ${pref.wa_id} opted out of marketing`);
        }
    }
});
```
