...
| `appId` | `string` | `undefined` | Meta for Developers application ID |
| `appSecret` | `string` | `undefined` | Meta for Developers application secret |
| `webhookVerificationToken` | `string` | `undefined` | Token for webhook URL verification |
| `requestTimeout` | `number` | `undefined` | Request timeout in milliseconds |
| `retry` | `RetryConfig` | `undefined` | Automatic retry config for throttling errors |
...
Configure automatic retries for throttling errors (rate limits). When the WhatsApp API returns a `WhatsAppThrottlingError`, the SDK automatically retries with backoff:
...
const client = new WhatsApp({
    ...
    phoneNumberId: Number(process.env.WA_PHONE_NUMBER_ID),
    retry: {
        maxAttempts: 3,           // Including initial attempt (default: 3)
        backoff: 'exponential',   // 'exponential' or 'fixed' (default: 'exponential')
        initialDelayMs: 1000,     // Delay before first retry (default: 1000)
    },
});
```
...
| Strategy | Behavior |
|---|---|
| `exponential` (default) | Delay doubles each attempt: 1s → 2s → 4s → ... |
