...
const emptyConfigChecker = (senderNumberId?: number, accessToken?: string) => {
    if (!process.env[WabaConfigEnum.PhoneNumberId] && !senderNumberId) {
...
export const importConfig = (senderNumberId?: number, accessToken?: string) => {
    emptyConfigChecker(senderNumberId, accessToken);
