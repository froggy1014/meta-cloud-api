...
describe('PhoneNumber API - Unit Tests', () => {
    ...
    describe('PhoneNumber Operations', () => {
        it('should get phone number by ID with correct endpoint', async () => {
            await whatsApp.phoneNumber.getPhoneNumberById();
        ...
        it('should get phone number by ID with fields parameter', async () => {
            ...
            await whatsApp.phoneNumber.getPhoneNumberById(fields);
        ...
        it('should get all phone numbers with correct endpoint', async () => {
            await whatsApp.phoneNumber.getPhoneNumbers();
        ...
        it('should request verification code with correct endpoint and body', async () => {
            ...
            await whatsApp.phoneNumber.requestVerificationCode(request);
        ...
        it('should verify code with correct endpoint and body', async () => {
            ...
            await whatsApp.phoneNumber.verifyCode(request);
    ...
    describe('Query Parameter Handling', () => {
        it('should handle single field parameter correctly', async () => {
            ...
            await whatsApp.phoneNumber.getPhoneNumberById(field);
    ...
    describe('Error Handling', () => {
        it('should handle API errors gracefully for getPhoneNumberById', async () => {
            ...
            await expect(whatsApp.phoneNumber.getPhoneNumberById()).rejects.toThrow(
        ...
        it('should handle API errors gracefully for getPhoneNumbers', async () => {
            ...
            await expect(whatsApp.phoneNumber.getPhoneNumbers()).rejects.toThrow(
        ...
        it('should handle API errors gracefully for requestVerificationCode', async () => {
            ...
            await expect(whatsApp.phoneNumber.requestVerificationCode(request)).rejects.toThrow(
        ...
        it('should handle API errors gracefully for verifyCode', async () => {
            ...
            await expect(whatsApp.phoneNumber.verifyCode(request)).rejects.toThrow(
        ...
        it('should handle network timeout errors', async () => {
            ...
            await expect(whatsApp.phoneNumber.getPhoneNumberById()).rejects.toThrow('Network timeout');
            await expect(whatsApp.phoneNumber.getPhoneNumbers()).rejects.toThrow('Network timeout');

            const verificationRequest = { code_method: 'SMS' as const, language: 'en_US' };
            await expect(whatsApp.phoneNumber.requestVerificationCode(verificationRequest)).rejects.toThrow(
                'Network timeout',
            );

            const verifyRequest = { code: '123456' };
            await expect(whatsApp.phoneNumber.verifyCode(verifyRequest)).rejects.toThrow('Network timeout');
        });
    });

    describe('API Integration', () => {
        it('should be accessible through WhatsApp instance', () => {
            expect(whatsApp.phoneNumber).toBeDefined();
            expect(typeof whatsApp.phoneNumber.getPhoneNumberById).toBe('function');
            expect(typeof whatsApp.phoneNumber.getPhoneNumbers).toBe('function');
            expect(typeof whatsApp.phoneNumber.requestVerificationCode).toBe('function');
            expect(typeof whatsApp.phoneNumber.verifyCode).toBe('function');
        });
        ...
        it('should use phone number ID in single resource endpoints', async () => {
            ...
            mockRequestSend.mockClear();
            await whatsApp.phoneNumber.requestVerificationCode({ code_method: 'SMS', language: 'en_US' });
            ...
            // Test verifyCode
            mockRequestSend.mockClear();
            await whatsApp.phoneNumber.verifyCode({ code: '123456' });
    ...
    describe('Request Consistency', () => {
        it('should always include required fields in verification code requests', async () => {
            ...
            for (const testCase of testCases) {
                mockRequestSend.mockClear();
                await whatsApp.phoneNumber.requestVerificationCode(testCase);
        ...
        it('should always include only code field in verify requests', async () => {
            ...
            for (const code of codes) {
                mockRequestSend.mockClear();
                await whatsApp.phoneNumber.verifyCode({ code });
        ...
        it('should never include body for GET requests', async () => {
            ...
            mockRequestSend.mockClear();
            await whatsApp.phoneNumber.getPhoneNumberById('display_phone_number');
            expect(mockRequestSend.mock.calls[0][3]).toBeNull();
